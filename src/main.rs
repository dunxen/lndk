...
async fn main() -> Result<(), ()> {
    ...
    for peer in current_peers.into_inner().peers {
        ...
        let onion_support = match client
            .lightning()
    ...
    CMH::Target: CustomOnionMessageHandler + Sized,
{
    ...
    // Consume events is our main controlling loop, so we run it inline here. This function is expected to finish if
    // any producing thread exits (because we're not longer receiving the events we need).
                    ...
                    Ok(peer_event) => match peer_event.r#type() {
                        PeerOnline => {
                            ...
                            match onion_support {
                                ...
                                // If we couldn't lookup the peer's onion message support, just log the error and
                                // ignore the connection event.
                                Err(_) => {
...
mod tests {
    ...
    fn test_consume_messenger_events() {
        ...
        block_on(sender.send(MessengerEvents::PeerConnected(pk, true))).unwrap();
        ...
        block_on(sender.send(MessengerEvents::PeerConnected(pk, false))).unwrap();
        ...
        block_on(sender.send(MessengerEvents::PeerDisconnected(pk))).unwrap();
        mock.expect_peer_disconnected().return_once(|_| ());

        // Finally, send a producer exit event to test exit.
        block_on(sender.send(MessengerEvents::ProducerExit(
            ConsumerError::PeerProducerExit,
        )))
        .unwrap();

        let consume_err =
            block_on(consume_messenger_events(mock, receiver)).expect_err("consume should error");
    ...
    fn test_consumer_clean_exit() {
        ...
        assert!(block_on(consume_messenger_events(
    ...
    fn test_produce_peer_events() {
        ...
        mock.expect_onion_support().returning(|_| Err(()));
        ...
        matches!(
            block_on(produce_peer_events(mock, sender, exit)).expect_err("producer should error"),
    ...
    #[test]
    fn test_produce_peer_events_exit() {
        let (sender, _receiver) = channel(1);
        let (exit_sender, exit) = channel(1);

        let mock = MockPeerProducer::new();
        drop(exit_sender);
        assert!(block_on(produce_peer_events(mock, sender, exit)).is_ok());
    }
}
